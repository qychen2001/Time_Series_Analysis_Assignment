...
\begin{array}{c}
    ...
    \frac{\partial}{\partial\mu}l=-\frac{1}{2\sigma_e^2}\sum_{t=2}^n e_t\frac{\partial}{\partial\mu}e_t+\frac{(1-\phi^2)(Y_1-\mu)}{\sigma_e^2}\\
    \frac{\partial}{\partial\sigma_e^2}l=-\frac{n}{2\sigma_e^2}+\frac{1}{2\sigma_e^4}\sum_{t=2}^n e_t^2\nonumber
\end{array}
\right.
\end{equation}
...
\end{lstlisting}
\begin{lstlisting}[language=R]
> clse.arma(ar2.s,c(2,0),LearningRate = 1,intercept = F)
`Local minimum found.`
       ar1        ar2
 1.5152597 -0.8046584
> arima(ar2.s,c(2,0,0),method="CSS",include.mean = F)

Call:
arima(x = ar2.s, order = c(2, 0, 0), include.mean = F, method = "CSS")
...
sigma^2 estimated as 0.8772:  part log likelihood = -162.41
\end{lstlisting}

以下对ma2.s数据集进行测试
\begin{lstlisting}[language=R]
> clse.arma(ma2.s,c(0,2),LearningRate = 0.1)
`Local minimum found.`
       ma1        ma2  intercept
 1.0559739 -0.5722273  0.1352184
> arima(ma2.s,c(0,0,2),method="CSS")

Call:
arima(x = ma2.s, order = c(0, 0, 2), method = "CSS")

Coefficients:
          ma1     ma2  intercept
      -1.0560  0.5723     0.1352
s.e.   0.0873  0.0863     0.0511

sigma^2 estimated as 1.184:  part log likelihood = -180.42
\end{lstlisting}
\begin{lstlisting}[language=R]
> clse.arma(ma2.s,c(0,2),LearningRate = 0.1,intercept = F)
`Local minimum found.`
       ma1        ma2
 1.0300179 -0.5775751
> arima(ma2.s,c(0,0,2),method="CSS",include.mean = F)

Call:
arima(x = ma2.s, order = c(0, 0, 2), include.mean = F, method = "CSS")
...
sigma^2 estimated as 1.25:  part log likelihood = -183.64
\end{lstlisting}

以下对arma11.s数据集进行测试
\begin{lstlisting}[language=R]
> clse.arma(arma11.s,c(1,1),LearningRate = 0.1)
`Local minimum found.`
       ar1        ma1  intercept
 0.5585807 -0.3668805  0.3923015
> arima(arma11.s,c(1,0,1),method="CSS")
...
\end{lstlisting}
\begin{lstlisting}[language=R]
> clse.arma(arma11.s,c(1,1),LearningRate = 0.1,intercept = F)
`Local minimum found.`
       ar1        ma1
 0.5874466 -0.3471722
> arima(arma11.s,c(1,0,1),method="CSS",include.mean = F)

Call:
arima(x = arma11.s, order = c(1, 0, 1), include.mean = F, method = "CSS")
...
sigma^2 estimated as 1.215:  part log likelihood = -151.62
\end{lstlisting}
...
`Local minimum found.`
      ar1 intercept
0.8922657 1.2520913
> arima(ar1.s,c(1,0,0),method="ML")

Call:
arima(x = ar1.s, order = c(1, 0, 0), method = "ML")
...
\end{lstlisting}
\begin{lstlisting}[language=R]
> mle.ar1(ar1.s,intercept = F)
`Local minimum found.`
      ar1
0.9244036
> arima(ar1.s,c(1,0,0),method="ML",include.mean = F)

Call:
arima(x = ar1.s, order = c(1, 0, 0), include.mean = F, method = "ML")
...
sigma^2 estimated as 1.048:  log likelihood = -87.52,  aic = 177.04
\end{lstlisting}
...
> me.ar(ar2.s,2)
       ar1        ar2
 1.4694476 -0.7646034
> ar.yw(ar2.s,order.max = 2)

Call:
ar.yw.default(x = ar2.s, order.max = 2)
...
\noindent(1)\quad uls.ar1(data)与arima(data,order=c(1,0,0))。考虑到arima函数提供的method选项只有"CSS","ML","CSS-ML"三种，故认为如果可以，则"CSS-ML"应该对应了无条件最小二乘法。以下对ar1.s数据集进行测试
...
`Local minimum found.`
      ar1 intercept
0.8610367 1.4113062
> arima(ar1.s,c(1,0,0),method = "CSS-ML")

Call:
arima(x = ar1.s, order = c(1, 0, 0), method = "CSS-ML")
...
\end{lstlisting}
\begin{lstlisting}[language=R]
> ulse.ar1(ar1.s,intercept = F)
`Local minimum found.`
      ar1
0.9283047
> arima(ar1.s,c(1,0,0),method = "CSS-ML",include.mean = F)

Call:
arima(x = ar1.s, order = c(1, 0, 0), include.mean = F, method = "CSS-ML")
...
\noindent(2)\quad me.arma11(data)与arima(data,order=c(1,0,0))。在arima函数的method选项中没有矩估计相关的方法种类，而R语言中自带的时间序列参数估计函数只有arima一个可以估计arma序列参数，故理论上me.arma11就应该找不到对应的库函数。

\begin{lstlisting}[language=R]
> me.arma11(arma11.s)
       ar1        ma1
 0.6377807 -0.2038076
> arima(arma11.s,c(1,0,1),include.mean = F)

Call:
arima(x = arma11.s, order = c(1, 0, 1), include.mean = F)
...
sigma^2 estimated as 1.207:  log likelihood = -151.76,  aic = 307.52
\end{lstlisting}
